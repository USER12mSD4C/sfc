...
impl Sha256 {
    ...
    fn update(&mut self, data: &[u8]) {
        ...
        if buffer_len > 0 {
            ...
            if input.len() >= fill {
                self.buffer[buffer_len..64].copy_from_slice(&input[..fill]);
                // Копируем буфер на стек во избежание конфликта Borrow Checker
        ...
        let _ = buffer_len; // Подавляем предупреждение об unused_assignments
    }

    fn finalize(mut self) -> [u8; 32] {
        let bit_len = self.count << 3;
        let buffer_len = (self.count & 0x3f) as usize;
        ...
        for i in 0..8 {
            pad[pad_len + i] = (bit_len >> (56 - i * 8)) as u8;
        }
                ...
                Ok(mut file) => {
                    ...
                    loop {
                        match file.read(&mut buffer) {
                            ...
                            Err(e) => {
                                eprintln!("sha256sum: {}: {}", arg, e);
                                break;
                            }
                        }
                    }
                    let hash = hasher.finalize();
                    println!("{}  {}", hex_string(&hash), arg);
                }
                Err(e) => {
                    eprintln!("sha256sum: {}: {}", arg, e);
                }
